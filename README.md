...
| `user-principal-name` | Variable | Usually configuration, not a secret. |
...
`auth-cookie` is masked before being written as an output, but it is still sensitive session material. The action prefers `ESTSAUTHPERSISTENT` when available because that is the most compatible cookie for downstream consumers such as XDR portal flows.
